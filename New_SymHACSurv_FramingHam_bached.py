...
from survival import MixExpPhiStochastic, InnerGenerator, InnerGenerator2, HACSurvival_competing_shared, sample,HACSurv_3D_Sym_shared
...
def run_experiment(seed, base_path):
    ...
    model =HACSurv_3D_Sym_shared(phi, device = device, num_features=18, tol=1e-14, hidden_size = 100).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 20 == 0:
            ...
            if avg_conditional_c_index > best_avg_c_index:
                ...
                best_model_filename = f"New_HACSurv(Sym)_shared_Framing2_Risks_cindex_{best_avg_c_index:.4f}_{current_time()}_{seed}.pth"
                ...
                plt.savefig('./figures/symHACSurv_FramingHam.png' )
                plt.clf()
...
# 汇总并打印所有种子的测试结果
print("\n=== Summary of Test Results of HACSurv(SYM) Across All Seeds ===")
