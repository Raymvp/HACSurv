...
# 导入自定义模块
from survival import MixExpPhiStochastic,InnerGenerator,InnerGenerator2,sample,HACSurvival_competing5,HACSurvival_competing5_shared
...
def process_surv_df_jointCIF(surv_df, step):
...
def run_experiment(seed, base_path):
    ...
    phi_24= InnerGenerator(psi,device)
    phi_135= InnerGenerator(psi,device)

    model =HACSurvival_competing5_shared(psi,phi_24,phi_135, device = device, num_features=40, tol=1e-14, hidden_size = 100).to(device)

    ckpt_path_24 = './MIMIC-III/checkpoint/Model_inner_e2e4_step205outer.pth'
    ckpt_24 = torch.load(ckpt_path_24)
    ckpt_path_135 = './MIMIC-III/checkpoint/Model_inner_e1e3e5_step205outer.pth'
    ckpt_135 = torch.load(ckpt_path_135)

    phi_24_keys =  {k.replace('phi.', ''): v for k, v in ckpt_24['model_state_dict'].items() if 'phi' in k and 'phi_inv' not in k}
    model.phi_24.load_state_dict(phi_24_keys)
    phi_24_inv_keys = {k.replace('phi_inv.', ''): v for k, v in ckpt_24['model_state_dict'].items() if 'phi_inv.' in k}
    model.phi_24_inv.load_state_dict(phi_24_inv_keys)

    phi_135_keys =  {k.replace('phi.', ''): v for k, v in ckpt_135['model_state_dict'].items() if 'phi' in k and 'phi_inv' not in k}
    model.phi_135.load_state_dict(phi_135_keys)
    phi_135_inv_keys = {k.replace('phi_inv.', ''): v for k, v in ckpt_135['model_state_dict'].items() if 'phi_inv.' in k}
    model.phi_135_inv.load_state_dict(phi_135_inv_keys)
    ...
    event_ranges = [range(5), range(5), range(5), range(5)]  # 每个方法对应五个风险
    surv_df_processings = [
        process_surv_df_CIF,
        process_surv_df_jointCIF,  
        process_surv_df_jointCIF,
        process_surv_df_SF
    ]
    ...
    test_metrics = {
        ...
        for method_name in ["Conditional CIF", "Joint CIF", "Conditional CIF Integral", "Marginal SF"]
    }
    ...
    for epoch in range(num_epochs):
        ...
        for covariates, times, events in train_loader:
            # 训练过程
            model.phi_24.psi.resample_M(100)
            model.phi_24.resample_M(100)
            model.phi_135.resample_M(100)
        ...
        if epoch % 100 == 0:
            ...
            if avg_conditional_c_index > best_avg_c_index:
                ...
                best_model_filename = f"05outerNew_HACSurv_shared_MIMIC_cindex_{best_avg_c_index:.4f}_{current_time()}_{seed}.pth"
...
# 设置保存模型的基础路径
base_path = "/home/liuxin/HACSurv_Camera_Ready/MIMIC-III/checkpoint"
...
# 汇总并打印所有种子的测试结果
print("\n=== Summary of Test Results of HACSurv(05Outer) Across All Seeds ===")
