...
from survival import HACSurv_2D,MixExpPhiStochastic
...
def main():
    for theta_true in [2,5,10]:
        ...
        model = HACSurv_2D(phi, device = device, num_features=10, tol=1e-14).to(device)
        ...
        #                         ])
        # optimizer_copula = optim.SGD([{"params": model.phi.parameters(), "lr": 0.0001}])
        ...
        checkpoint_dir = './checkpoints/HACSurv/'
        sample_figs_dir = './single_figure/HACSurv/' + copula_form + '/' + str(theta_true) + '/'
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 1 == 0:
                ...
                if val_loglikelihood > best_val_loglikelihood:
                    ...
                    }, './checkpoints/HACSurv/checkpoint_experiment_'+copula_form + '_' +str(theta_true)+'.pth')
            ...
            if epoch % 200 == 0:
                ...
                plt.savefig('./single_figure/HACSurv/'+copula_form+'/'+str(theta_true)+'/epoch%s.png' %
                            (epoch))
                plt.clf()

        checkpoint = torch.load('./checkpoints/HACSurv/checkpoint_experiment_'+copula_form + '_' +str(theta_true)+'.pth')
        model.load_state_dict(checkpoint['model_state_dict'])
        samples =  sample(model, 2, 2000, device =  device)
        plt.scatter(samples[:, 0].cpu(), samples[:, 1].cpu(), s = 15)
        plt.savefig('./single_figure/HACSurv/'+copula_form+'/'+str(theta_true)+'/best_epoch.png')
        plt.clf()
